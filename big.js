window.onload = function() {
  ...
  if (!s.length) return;
  var big = window.big = { current: 0, forward: fwd, reverse: rev, go: go, length: s.length, times: times, elapsed: elapsed, startTime: Date.now() };
  setInterval(elapsed, 2 * 60 * 1000)
  function resize() {
  ...
  function go(n) {
    if (big.previous) {
      var elapsed = ((Date.now() - big.currentStartTime) / 60000).toFixed(2)
      s[big.previous].setAttribute('data-elapsed', elapsed)
    }
    big.current = big.previous = n;
    big.currentStartTime = Date.now();
  ...
  function times() {
    for (i = 0; i < s.length; i++) console.log('Slide ' + i, s[i].getAttribute('data-elapsed'));
  }
  function elapsed() {
    console.log('%cElapsed: %s', 'color: blue', ((Date.now() - big.startTime) / 60000).toFixed(2))
  }
